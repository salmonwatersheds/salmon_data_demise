...
## Model dredging + averaging

The objectives are exploratory rather than confirmatory.

```{r}
# https://bookdown.org/danielnettle2/bookdown/model-selection-and-model-averaging.html
library(MuMIn)

glm_full_i <- glm.nb(count_pop ~ count_catch_sqrt * value_kg_sqrt * year * species, data = data, link = "log")

options(na.action = "na.fail")
results <- model.sel(object = dredge(glm_full_i, beta = "none"), beta = "none")
options(na.action = "na.omit")

subset(results, delta < 10)
```

Select the model in the 0.95% confidence set:

```{r}
models.selected <- get.models(results, subset = cumsum(weight) <= .95)
length(models.selected)
```

Model `glm_full_i` is the only model in the 0.95 confidence set.


# Include stream_survey_quality

We determined the number of populations surveyed per `species` and `stream_survey_quality`:

```{r, echo=FALSE}
head(data_sp_ssq)
```

The proportions of the different values of `stream_survey_quality` WITH `count_pop` = 0:

```{r, echo=FALSE}
sapply(X = unique(data_sp_ssq$stream_survey_quality),function(ssq){
  if(is.na(ssq)){
    cond <- is.na(data_sp_ssq$stream_survey_quality)
  }else{
    cond <- !is.na(data_sp_ssq$stream_survey_quality) & data_sp_ssq$stream_survey_quality == ssq
  }
  out <- round(sum(cond)/nrow(data_sp_ssq),2)
})
```

The proportions of the different values of `stream_survey_quality` WITHOUT `count_pop` = 0:

```{r, echo=FALSE}
cond_0no <- data_sp_ssq$count_pop != 0
sapply(X = unique(data_sp_ssq$stream_survey_quality[cond_0no]),function(ssq){
  if(is.na(ssq)){
    cond <- is.na(data_sp_ssq$stream_survey_quality[cond_0no])
  }else{
    cond <- !is.na(data_sp_ssq$stream_survey_quality[cond_0no]) & data_sp_ssq$stream_survey_quality[cond_0no] == ssq
  }
  out <- round(sum(cond)/nrow(data_sp_ssq[cond_0no,]),4)
})
```

We can see in the figure below that `stream_survey_quality` started to be consistently reported from 1998. We consequently only consider data from this date.

```{r, echo=FALSE, fig.cap = "Number of population among the stream survey quality categories over time", out.width = '100%'}
knitr::include_graphics(paste0(wd,"/figures/Barplot_EstimateClassification.jpeg"))
```

We remove NA and "Unknown" values and data before 1998; the proportion of zeros counts is now very small:

```{r, echo=FALSE}
# data_sp_ssq <-  read_xlsx(paste0(wd_data_output,"/",filename,".xlsx"), 
#                          sheet = "populations_species_ssq") |> as.data.frame()

cond_NA <- is.na(data_sp_ssq$stream_survey_quality)
cond_uk <- !cond_NA & data_sp_ssq$stream_survey_quality == "Unknown"
cond_1998 <- data_sp_ssq$year >= 1998
data_sp_ssq <- data_sp_ssq[!cond_NA & !cond_uk & cond_1998,]

round(sum(data_sp_ssq$count_CU == 0)/nrow(data_sp_ssq),4)
```

Let's look at the change in monitoring per `stream survey quality` categories (observations with `MAX_ESTIMATE` = 0 removed):

```{r, echo=FALSE}
data_here <- data_sp_ssq

years <- sort(unique(data_here$year))

species <- unique(data_here$species)
...
Let's look at the change in monitoring per `stream survey quality` categories (observations with `MAX_ESTIMATE` = 0 present):

```{r, echo=FALSE}

data_sp_ssq_with0 <-  read_xlsx(paste0(wd_data_output,"/populationAssessed_catches_data_remove_NAs.xlsx"),
                         sheet = "populations_species_ssq") |> as.data.frame()

cond_NA <- is.na(data_sp_ssq_with0$stream_survey_quality)
cond_uk <- !cond_NA & data_sp_ssq_with0$stream_survey_quality == "Unknown"
cond_1998 <- data_sp_ssq_with0$year >= 1998
data_sp_ssq_with0 <- data_sp_ssq_with0[!cond_NA & !cond_uk & cond_1998,]

data_here <- data_sp_ssq_with0
...
We now create the categorical variable `stream_survey_quality_cat` such as:

```{r, echo=FALSE}
stream_survey_quality_cat <- 1:5
names(stream_survey_quality_cat) <- c("Low","Medium-Low","Medium","Medium-High","High")
data_sp_ssq$stream_survey_quality_cat <- NA
for(ssq in unique(data_sp_ssq$stream_survey_quality)){
  cond <- data_sp_ssq$stream_survey_quality == ssq
  data_sp_ssq$stream_survey_quality_cat[cond] <- stream_survey_quality_cat[ssq]
}
data_sp_ssq$stream_survey_quality_cat <- factor(data_sp_ssq$stream_survey_quality_cat, levels = 1:5)
stream_survey_quality_cat
```

After adding the catch and value dataset, we have:

```{r, include=FALSE}
dataset_selected <- data_sp_ssq

year_min <- max(min(dataset_selected$year),min(catch$year),min(value_sp$Year))
year_max <- min(max(dataset_selected$year),max(catch$year),max(value_sp$Year))
years <- year_min:year_max

col_selected <- c("species","year","count_pop","stream_survey_quality_cat")
if(any(grepl("region",colnames(dataset_selected)))){
  col_selected <- c("region","species","year","count_pop","stream_survey_quality_cat")
}
...
```{r, echo=FALSE}
pairs(data[,c("count_pop","year","count_catch","value_kg","stream_survey_quality_cat")],
      upper.panel = upper.panel,
      lower.panel = panel.cor,
      diag.panel =  panel.hist)
```

We log transform `count_catch` and `value_kg` to reduce their skewness:

```{r, echo=FALSE}
data$count_catch_log <- log(data$count_catch)
data$value_kg_log <- log(data$value_kg)

pairs(data[,c("count_pop","year","count_catch_log","value_kg_log","stream_survey_quality_cat")],
      upper.panel = upper.panel,
      lower.panel = panel.cor,
      diag.panel =  panel.hist)
```

Let's fit the most complex model and alternative ones:

```{r}
glm_full_i <- glm.nb(count_pop ~ count_catch_log * value_kg_log * year * species + 
                       stream_survey_quality_cat + stream_survey_quality_cat:year + stream_survey_quality_cat:species, 
                     data = data, link = "log")

glm_cys_i_qys_i <- glm.nb(count_pop ~ count_catch_log * year * species + 
                       stream_survey_quality_cat + stream_survey_quality_cat:year + stream_survey_quality_cat:species, 
                     data = data, link = "log")

glm_syq_i <- glm.nb(count_pop ~ species * year * stream_survey_quality_cat, 
                     data = data, link = "log")
```
...
list_glm <- list(glm_full_i,glm_cys_i_qys_i,glm_syq_i)

names(list_glm) <- c("glm_full_i","glm_cys_i_qys_i","glm_syq_i")
...
Looks like we do not need `count_catch` and `value_kg`...

Check of the residuals of `glm_full_i`:
...
x <- data$count_catch_log
plot(y = res, x = x, xlab = "log(Catch)", ylab = "Deviance residuals")
abline(0,0)
smoothed <- predict(loess(res ~ x, span = span))
lines(x = x[order(x)], y = smoothed[order(x)], col = "red")
...
#
x <- data$value_kg_log
plot(y = res, x = x, xlab = "log(Value) (in CAD per kg)", ylab = "Deviance residuals")
abline(0,0)
smoothed <- predict(loess(res ~ x, span = span))
lines(x = x[order(x)], y = smoothed[order(x)], col = "red")
...
x <- factor(data$stream_survey_quality_cat, levels = 1:5)
boxplot(res ~ x, xlab = "Stream survey quality", ylab = "Deviance residuals")
abline(0,0)
```
...
The are many models in the 0.95% confidence set:
...
The are only a few models with delta AIC <= 2:

```{r}
models.selected <- get.models(results, subset = delta <= 2)
length(models.selected)
```
...
```{r}
model_average <- model.avg(models.selected)
summa <- summary(model_average)
summa$coefmat.full
```

```{r, echo=FALSE}
model.avg.output.full <-  as.data.frame(cbind(
  summary(model_average)$coefmat.full[,c(1,3)],
  confint(model_average,full=T,level=0.95)))

model.avg.output.full$Estimate <- round(model.avg.output.full$Estimate,2)
model.avg.output.full$`Adjusted SE` <- round(model.avg.output.full$`Adjusted SE`,3)
model.avg.output.full$`2.5 %` <- round(model.avg.output.full$`2.5 %`,3)
model.avg.output.full$`97.5 %` <- round(model.avg.output.full$`97.5 %`,3)
model.avg.output.full
```

Prediction of the "full" averaged model (coefficients are averaged considering 0s when they are not present in model):
...
for(sp in unique(data$species)){
  ...
  cond_yr <- data$year == year
  
  for(ssq in 1:5){
    # ssq <- 5
    
    value_kg <- data$value_kg[cond_yr & cond_sp] |> unique()
    
    data_new <- data.frame(species = sp,
                           year = year,
                           stream_survey_quality_cat = ssq,
                           value_kg = value_kg,
                           value_kg_log = log(value_kg),
                           count_catch = x_range,
                           count_catch_log = log(x_range))
    
    data_new$stream_survey_quality_cat <- factor(data_new$stream_survey_quality_cat, 
                                                  levels = 1:5)
    
    # pred <- data.frame(
    #   model = sapply(models.selected, predict, newdata = data_new),
    #   averaged.subset = predict(model_average, data_new, full = F),
    #   averaged.full = predict(model_average, data_new, full = TRUE)
    # )
    
    fit_se <- predict(object = model_average, full = T,
                      newdata = data_new, 
                      type = "response", se.fit = T)
    ...
    polygon(x = c(data_new$count_catch,rev(data_new$count_catch)), 
            y = c(CI_low,rev(CI_high)), border = NA,
            col = colour_transparency_fun(cols_ssq[as.character(ssq)],alpha = .2))
    
    lines(x = data_new$count_catch, y = fit_se$fit, col = cols_ssq[as.character(ssq)], lwd = 2)
  }
  count <- count + 1
}
plot(1, type = "n", axes = FALSE, xlab = "", ylab = "")
legend("center",legend = rev(c("Low","Medium-Low","Medium","Medium-High","High")),
       col = rev(cols_ssq), lwd = 2, bty = "n", title = "Stream survey quality")
legend("bottom",paste0("Year = ",year), bty = "n")
mtext(text = "Number of fish caught (in thousands)", side = 3, cex = .7)
```

# Altenative: predict monitoring probability

## With same variables + regions

The goal here is to predict the monitoring of individual populations as the function of `catch`, `value`, `year`, `species` and `region` using the cleaned NuSEDS data (we keep the 0s and the populations without a `cuid`).

We remove observations with `MAX_ESTIMATE` = NA, by keep the ones = 0.
...
#
col_selected <- c("region","SPECIES","Year","population_id","stream_survey_quality")
nuseds_cut <- nuseds[nuseds$Year %in% years,col_selected] |> unique()
colnames(nuseds_cut) <-  c("region","species","year","population_id","stream_survey_quality")
...
The response variable `monitored` is binary:

```{r, echo=FALSE}
show <- head(data)
show <- show[,colnames(show) != "stream_survey_quality"]
rownames(show) <- NULL
show
```

There are a majority of zeros in the dataset:
...
round(sum(!data$monitored)/nrow(data),3)
```

We fit a glm with a binomial distribution and clog-log link function, which is the appropriate function when there is more negative than positive outcomes and vice versa (Zuur et al., 2009).

```{r}
link <- c("logit","probit","cloglog")[3]

# glm_full_i <- glm(monitored ~ count_catch * value_kg * year * species * region,
...
All these models explain very little of the variance:
...
These models don't seem to explain much variation in monitoring...
