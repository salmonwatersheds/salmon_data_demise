...
```{r, include=FALSE}
year_min <- max(min(data_sp$year),min(catch$year),min(value_sp$Year))
year_max <- min(max(data_sp$year),max(catch$year),max(value_sp$Year))
years <- year_min:year_max

data <- data_sp[data_sp$year %in% years,c("species","year","count_pop")]
data$count_catch <- NA
data$value_kg <- NA
for(yr in years){
  # yr <- years[1]
  for(sp in unique(data_sp$species)){
...
The response variable is a count, hence fitting a GLM with a Poisson distribution is suggested but simple LM (assuming a Gaussian distribution of the response variable) can be enough if counts are far from 0s. So we try a LM first.
...
```{r}
# lm_full_i <- lm(count_pop ~ count_catch * value_kg * year * species, data = data)
...
I use the variance inflation factor (VIF) and the generalized VIF (GVIF`^(1/(2*Df))`) to check the collinearity (the latter being used when `species` is present) in the models wihout interactions. (GVIF`^(1/(2*Df))` with the model with interactions is always = 1 because the interactions concern all the explanatory variables.)
...
Most parsimonious model: `lm_sy_i`, according to AIC.
...
```{r}
# glm_full_i <- glm(count_pop ~ count_catch * value_kg * year * species, data = data, family = poisson)
...
show$R2_McFadden <- lapply(X = list(glm_full,
                                    glm_cv_i,
                                    glm_cv,
                                    glm_c,
                                    glm_sy_i,
...
show$R2_pseudo <- lapply(X = list(glm_full,
                                  glm_cv_i,
                                  glm_cv,
                                  glm_c,
                                  glm_sy_i,
...
Most parsimonious model: `glm_sy_i`, according to AIC.
...
There are patterns in the residuals for both models.
...
Until now I am not convinced that only `species` and `year` have a significant effect on `count_pop`. I do not think we have the data to formulate clear and distints hypotheses to validate or invalidate the effect of catch on monitoring.

This is not a controlled experiment and there are certainly more factors at play than the ones considered in our models. Consequently the question should not be "does catch has an effect of not on monitoring" because we know it does, yet our approach kind invalidates it but in a week way.
...
show$R2_McFadden <- lapply(X = list(glm_full_i,
                                    glm_full,
                                    glm_cv_i,
                                    glm_cv,
                                    glm_c,
...
# pseudo-R2
show$R2_pseudo <- lapply(X = list(glm_full_i,
                                  glm_full,
                                  glm_cv_i,
                                  glm_cv,
                                  glm_c,
...
The full model is buy far the best one according to AIC.
