---
title: "Supporting_information (Supplement B)"
...
The is the supporting information for: [Atkinson et al., 2025. Monitoring for fisheries or for fish? Declines in monitoring of salmon spawners continue despite a conservation crisis](https://cdnsciencepub.com/doi/10.1139/cjfas-2024-0387).

The document contains *(i)* the supplementary figures and tables referred to in the main text, *(ii)* the details of the generalized linear regression analysis conducted to quantify the effect of commercial catch, price on monitoring effort for each species, and *(iii)* a summary of the NuSEDS cleaning procedure. The entire NuSEDS cleaning procedure can be found at [Supplement A - Part 1](https://bookdown.org/salmonwatersheds/1_nuseds_collation_atkinson/1_nuseds_collation.html) and [Supplement A - Part 2](https://bookdown.org/salmonwatersheds/2_nuseds_cuid_pse_atkinson/2_nuseds_cuid_pse.html).
...
#'- populationAssessed_catches_data_remove_NAs.xlsx      # The summary files where only NAs counts were removed (results presented in the supporting information); created in 1_datasets.R 
#'- landed-value-valeur-debarquement-eng.csv             # price per kg and species from DFO’s 2023 Economic analysis
#' 
#' Files produced: 
#'- Predictinon_glm_WHITHOUT_0s.jpeg        # Figure 6 & S16         
#'- Predictinon_glm_WHITH_0s.jpeg           # Figure S22
#'
#' 
#'******************************************************************************
...
Below is a summary of NuSEDS cleaning procedure and attribution of the data to Conservation Units (CUs) as defined in the Pacific Salmon Explorer ([PSE](https://www.salmonexplorer.ca/)). The complete details of the cleaning procedure (**Supplement A - Part 1**) can be found at [a_nuseds_collation.html](https://bookdown.org/salmonwatersheds/1_nuseds_collation_atkinson/1_nuseds_collation.html) and the corresponding Rmd script can be accessed on [GitHub: a_nuseds_collation.Rmd](https://github.com/salmonwatersheds/salmon_data_demise/blob/main/code/a_nuseds_collation.Rmd) and [Zenodo: /code](https://zenodo.org/records/14248904). 

The detailed procedure to match the cleaned NuSEDS data to CUs (**Supplement A - Part 2**) is available at [b_nuseds_cuid_pse.html](https://bookdown.org/salmonwatersheds/2_nuseds_cuid_pse_atkinson/2_nuseds_cuid_pse.html) and the corresponding Rmd script can be accessed on [GitHub: b_nuseds_cuid_pse.Rmd](https://github.com/salmonwatersheds/salmon_data_demise/blob/main/code/b_nuseds_cuid_pse.Rmd) and [Zenodo: /code](https://zenodo.org/records/14248904). 

Note that the scripts **a_nuseds_collation.Rmd** and **b_nuseds_cuid_pse.Rmd** are permanent and specific to [Atkinson et al., 2025](https://cdnsciencepub.com/doi/10.1139/cjfas-2024-0387). The equivalent scripts updated as new NuSEDS versions are released are [1_nuseds_collation.rmd](https://github.com/salmonwatersheds/population-indicators/blob/master/spawner-surveys/code/1_nuseds_collation.rmd) and [2_nuseds_cuid_pse.rmd](https://github.com/salmonwatersheds/population-indicators/blob/master/spawner-surveys/code/2_nuseds_cuid_pse.Rmd).

The different versions of the final dataset **nuseds_cuid_streamid_DATE.csv** can be downloaded from [zenodo/.../2_nuseds_cuid_streamid](https://doi.org/10.5281/zenodo.14194638); the version used for the analysis in [Atkinson et al., 2025](https://cdnsciencepub.com/doi/10.1139/cjfas-2024-0387) is v3 (2025-04-15).
...
We matched the cleaned NuSEDS data with the CUs and streams defined in the Pacific Salmon Explorer, which are associated with a given region ([PSE](https://salmonexplorer.ca)). We used the fields `POP_ID`, `FULL_C_IN` and `CU_NAME`to match the CUs and the fields `SYSTEM_SITE`, `X_LONGT` and `Y_LAT` to match the streams. There are 23 CU_NAME not matching to any CUs in the PSE, corresponding to 87 populations and 2256 data points. These time series are kept in the dataset *2_nuseds_cuid_streamid.csv*. As mentionned above, add the details about the procedure can be found at [2_nuseds_cuid_pse.html](https://bookdown.org/salmonwatersheds/2_nuseds_cuid_pse_atkinson/2_nuseds_cuid_pse.html).

## Managing NA and zero counts

For the analysis presented in the main text, we excluded data points where `MAX_ESTIMATE` = NA (n = 156,507) or 0 (n = 3,449) and we present here in this supporting document the results of the analyses done on the cleaned NuSEDS dataset where zeros are kept.
