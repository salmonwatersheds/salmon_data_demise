---
title: "a_nuseds_collation (Supplement A - Part 1)"
...
This is the first (and most intensive) part of the cleaning procedure. The cleaned merge of **NUSEDS** and **CUSS** is exported at the end as *1_NuSEDS_escapement_data_collated_2025-04-15.csv*, which is used in the next part of the cleaning procedure.  

The second part [Supplement A - Part 2](https://bookdown.org/salmonwatersheds/2_nuseds_cuid_pse_atkinson/2_nuseds_cuid_pse.html) (script *b_nuseds_cuid_pse.rmd*) consists in attributing a CU identification number to each population (`cuid`) as defined in the Pacific Salmon Explorer ([PSE](https://www.salmonexplorer.ca/)). It is at the end of this second part that the file *nuseds_cuid_streamid_2025-04-15.csv* used for the analyses in [Atkinson et al. 2025. Monitoring for fisheries or for fish? Declines in monitoring of salmon spawners continue despite a conservation crisis](https://cdnsciencepub.com/doi/abs/10.1139/cjfas-2024-0387) is produced. The file can be accessed on Zenodo at [Cleaned spawner survey data v3](https://zenodo.org/records/15625993) and inside [salmon_data_demise/data_output](https://zenodo.org/records/14248904).
...
#'******************************************************************************
#' The goal of the script is to import, clean and format NuSEDS data into one unique 
#' dataset by merging all_area_nuseds (NUSEDS) and conservation_unit_system_sites (CUSS).

#' Imported files:

#'- all_areas_nuseds_2025-02-21.csv                    # "NUSEDS"
#'- conservation_unit_system_sites_2025-02-20.csv      # "CUSS"
...
#'- conservation_unit_report_definitions.csv           # definition of the fields in conservation_unit_system_sites
...
#'- 1_series_inNUSEDS_noInCUSS_2025-04-15.csv          # Export the series in NUSEDS with IndexIds and GFE_IDs not in CUSS with info on the number of data points and POPULATION
#'- 1_series_removed_2025-04-15.csv                    # series removed to either NUSEDS or CUSS
#'- 1_series_added_2025-04-15.csv                      # series added to either NUSEDS or CUSS
...
if(import_file_locally_TEMPORARY){
  ...
  if(remove_timeSeries_zeros_too){
      all_areas_nuseds <- read.csv(paste0(wd_data_output,"/all_areas_nuseds_noNA0s_TEMP.csv"),
                                   header = T)
  }else{
      all_areas_nuseds <- read.csv(paste0(wd_data_output,"/all_areas_nuseds_noNAs_TEMP.csv"),
                                   header = T)
  }
  ...
  if(remove_timeSeries_zeros_too){
      ...
      write.csv(all_areas_nuseds,paste0(wd_data_output,"/all_areas_nuseds_noNA0s_TEMP.csv"),
                row.names = F)
      ...
      write.csv(all_areas_nuseds,paste0(wd_data_output,"/all_areas_nuseds_noNAs_TEMP.csv"),
                row.names = F)
  }
}
...
if(import_file_locally_TEMPORARY){
  ...
  if(remove_timeSeries_zeros_too){
    ...
    trackRecord <- read.csv(paste0(wd_data_output,"/trackRecord_noNA0s_TEMP.csv"), header = T)
    
  }else{
    
    trackRecord <- read.csv(paste0(wd_data_output,"/trackRecord_noNAs_TEMP.csv"), header = T)
  ...
  if(remove_timeSeries_zeros_too){
      trackRecord <- cuss_nuseds_match_parallel_fun(conservation_unit_system_sites = conservation_unit_system_sites,
                                                ...
                                                cores_nb = cores_nb)
      write.csv(trackRecord,paste0(wd_data_output,"/trackRecord_noNA0s_TEMP.csv"), row.names = F)
      
  }else{
      trackRecord <- cuss_nuseds_match_parallel_fun(conservation_unit_system_sites = conservation_unit_system_sites,
                                                all_areas_nuseds = all_areas_nuseds,
                                                cores_nb = cores_nb)
      write.csv(trackRecord,paste0(wd_data_output,"/trackRecord_noNAs_TEMP.csv"), row.names = F)
  }
}
...
conservation_unit_system_sites <- conservation_unit_system_sites[!cond,]
nrow(conservation_unit_system_sites) # 6901 
...
if(export_datasets){
  ...
  write.csv(nuseds_final,paste0(wd_output,"/",name_file,".csv"),
            row.names = F)
  ...
  write.csv(trackRecord_toExport,paste0(wd_output,"/1_series_inNUSEDS_noInCUSS_",date,".csv"),
            row.names = F)
  ...
  write.csv(removed_all,paste0(wd_output,"/1_series_removed_",date,".csv"),row.names = F)
  ...
  write.csv(added_all,paste0(wd_output,"/1_series_added_",date,".csv"),row.names = F)
